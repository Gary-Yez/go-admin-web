<template>
  <el-card v-loading="sortSaving" class="container" shadow="never">
    <PageHeader description="管理导航结构、页面入口与显示顺序" title="菜单管理"/>
    <div @dragleave="handleDragLeave" @dragover="handleDragOver" @drop="handleDrop">
      <div v-if="listError" class="mb-[12px]">
        <el-alert :closable="false" :title="listError" show-icon type="error"/>
      </div>
      <ColumnTable ref="menuTableRef" v-loading="pageLoading" :data="tableData"
                   :row-class-name="rowClassName" row-key="id" size="large" storage-key="core/views/sys_menu/index:table-1">
        <template #toolbar>
          <el-button :loading="pageLoading" icon="Refresh" @click="getPageData">刷新</el-button>
          <el-button icon="Plus" type="primary" @click="()=>handleAdd({})">新增根菜单</el-button>
        ...
        <el-table-column label="字体图标" prop="icon">
          <template #default="{ row }">
            <div class="flex items-center gap-x-[8px]">
              <iconify-icon :icon="row.icon" class="text-[24px]"></iconify-icon>
              <span>{{ row.icon }}</span>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="路由地址" prop="path">
          <template #default="{ row }"><code class="table-code">{{ row.path }}</code></template>
        </el-table-column>
        ...
        <el-table-column :width="100" label="父菜单ID" prop="parent_id">
          <template #default="{ row }">
            <el-tag v-if="!row.parent_id" type="primary">根菜单</el-tag>
            <el-tag v-else type="success">{{ row.parent_id }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column :width="100" label="是否隐藏" prop="hidden">
          <template #default="{ row }">
            <el-switch v-model="row.hidden" :before-change="()=>handleChangeSwitch(row,'hidden')"
        ...
        <el-table-column :width="400" align="center" fixed="right" label="操作">
          <template #default="{ row }">
            <div class="table-btn-group">
              <el-button :aria-label="`拖动排序：${row.name}`" :disabled="sortSaving" :draggable="!sortSaving" class="drag-handle" plain
                         size="small" title="拖动调整位置或父级" type="primary"
                         @dragend="clearDrag" @dragstart="handleDragStart($event, row)">
                <el-icon class="mr-[4px]">
                  <iconify-icon icon="iconoir:drag"/>
                </el-icon>
                排序
              </el-button>
              <el-button :disabled="!!row.component" icon="Plus" plain size="small" type="primary"
                         @click="()=>handleAdd({parent_id:row.id})">添加子菜单
              </el-button>
        ...
        <template #empty>
          <el-empty v-if="!pageLoading" :description="listError ? '加载失败，请刷新重试' : '暂无菜单'" :image-size="70"/>
        </template>
      </ColumnTable>
    </div>
    <FormDialog v-model="dialogOpen" v-model:form="submitForm" :on-confirm="handleSubmit"
                :title="submitForm.id ? '修改菜单' : '新增菜单'" description="配置菜单名称、图标和页面入口，按需设置父级菜单。" note-icon="Menu">
      <el-form-item label="父菜单" prop="parent_id">
        <el-cascader v-model="submitForm.parent_id" :options="parentMenu" :props="{ label:'name',value:'id',checkStrictly:true,emitPath:false,disabled:'component'}" class="w-full"
                     size="large"></el-cascader>
      </el-form-item>
      <el-row :gutter="15">
        <el-col :span="10">
          <el-form-item :rules="[{required:true,message:'请选择字体图标'}]" label="字体图标" prop="icon">
            <IconSelect v-model="submitForm.icon"></IconSelect>
          </el-form-item>
        </el-col>
        <el-col :span="14">
          <el-form-item :rules="[{required:true,message:'请输入菜单名称'}]" label="菜单名称" prop="name">
            <el-input v-model="submitForm.name" placeholder="请输入菜单名称" size="large"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-form-item :rules="[{required:true,message:'请输入路由地址'}]" label="路由地址" prop="path">
        <el-input v-model="submitForm.path" placeholder="请输入路由地址" size="large">
      ...
      <el-row :gutter="12">
        ...
        <el-col v-if="submitForm.menu_type === 2" :span="13">
          <el-form-item :rules="[{required:true,message:'请选择组件'}]" label="组件路径" prop="component">
            <el-select v-model="submitForm.component" filterable placeholder="请选择组件" size="large">
              <el-option v-for="item in SyncComponents" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
      </el-row>
...
import {SysMenuApi} from "../../apis/sys_menu";
import FormDialog from "../../components/FormDialog.vue";
import {addSyncRouter, SyncComponents} from "../../routes/syncMenu";
...
const rowClassName = ({row}: {
  row: any
}) => `menu-row-${row.id} ${dragged.value?.id === row.id ? 'is-dragging' : ''} ${dropTarget.value === row.id ? `drop-${dropPosition.value}` : ''}`
...
function findMenu(id: number): any {
  return siblingsOf(id)?.find(row => row.id === id)
}
...
function menuLocation(menus: any[], matches: (row: any, path: string) => boolean, parent = '/dashboard'): {
  id: number;
  path: string
} | undefined {
...
function handleDragStart(event: DragEvent, row: any) {
  if (sortSaving.value || pageLoading.value || !event.dataTransfer) {
...
async function handleDrop(event: DragEvent) {
  event.preventDefault()
  if (!dragged.value || dropTarget.value === null || sortSaving.value) {
    clearDrag();
    return
  }
  ...
  destination.forEach((row: any, index: number) => {
    row.sort = index
  })
  ...
  try {
    ...
    if (router.currentRoute.value.fullPath === currentRoute.fullPath) {
      await router.replace({
        path: updated?.path || currentRoute.path,
...
:deep(.drop-inside > td) {
  ...
  box-shadow: inset 0 1px var(--el-color-primary), inset 0 -1px var(--el-color-primary);
}
