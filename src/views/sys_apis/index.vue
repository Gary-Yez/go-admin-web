<template>
  <el-card class="container" shadow="never">
    <PageHeader description="查看已同步的接口，维护分组说明与清理失效记录" title="API 管理"/>
    <el-form :model="searchForm" class="search-form" @submit.prevent="handleSearch">
      <el-input v-model="searchForm.keyword" clearable maxlength="100" placeholder="搜索路径或描述" prefix-icon="Search"
                size="large" style="--search-control-width: 260px" @blur="handleSearch"/>
      <el-select v-model="searchForm.method" clearable placeholder="全部方法" size="large"
                 style="--search-control-width: 140px" @change="handleSearch">
        <el-option v-for="method in ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'HEAD', 'OPTIONS', 'CONNECT', 'TRACE']"
                   :key="method" :label="method" :value="method"/>
      </el-select>
      <el-select v-model="searchForm.group" clearable filterable placeholder="全部分组" size="large"
                 style="--search-control-width: 210px" @change="handleSearch">
    ...
    <ColumnTable v-loading="pageLoading" :data="tableData" size="large"
                 storage-key="core/views/sys_apis/index:table-1" @sort-change="handleSortChange" @selection-change="handleSelectionChange">
      <template #toolbar>
        <el-button v-if="multipleSelection.length <= 0" :loading="pageLoading" icon="Refresh" @click="getPageData">
          刷新
        </el-button>
        <el-button v-if="multipleSelection.length <= 0" icon="Delete" type="primary" @click="handleCleanupAPI">清理失效
          API
        </el-button>
      ...
      <el-table-column label="路径" min-width="220" prop="path">
      ...
      <el-table-column :width="190" align="center" fixed="right" label="操作">
        <template #default="{ row }">
          <div class="table-btn-group">
            <el-button icon="Edit" plain size="small" type="primary" @click="()=>handleShowFormDialog(row)">修改
            </el-button>
      ...
      <template #empty>
        <el-empty v-if="!pageLoading"
                  :description="listError ? '加载失败，请刷新重试' : (queryForm.keyword || queryForm.method || queryForm.group) ? '没有匹配的API' : '暂无API'"
                  :image-size="70"/>
      </template>
    </ColumnTable>
    ...
    <FormDialog v-model="dialogOpen" v-model:form="submitForm" :on-confirm="handleSubmit"
                description="维护接口分组和描述，路径与请求方法由路由同步。" note-icon="Connection" title="修改 API 信息">
      <el-form-item :rules="[{required:true,message:'分组不能为空'}]" label="分组" prop="group">
        <el-select v-model="submitForm.group"
      ...
      <el-form-item :rules="[{required:true,message:'描述不能为空'}]" label="描述" prop="description">
        <el-input v-model="submitForm.description" placeholder="请输入描述" size="large"></el-input>
      </el-form-item>
    </FormDialog>
