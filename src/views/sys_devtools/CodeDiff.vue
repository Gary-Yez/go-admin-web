<template>
  <div class="code-diff">
    <div class="diff-toolbar">
      <div class="diff-counts"><span class="count-add">+ {{ added }} 行新增</span><span
          class="count-remove">− {{ removed }} 行删除</span><span
          v-if="!added && !removed">{{ before === after ? '内容相同' : '仅换行格式（CRLF / LF）不同' }}</span></div>
      <el-button size="small" text @click="toggleExpansion">{{
          hasExpansion ? '折叠未修改代码' : '展开全部代码'
        }}
      </el-button>
    </div>
    <div class="diff-panels">
      <section v-for="side in sides" :key="side" class="diff-panel">
        ...
        <div :ref="element => setScrollPane(side, element)" :aria-label="side === 'left' ? '当前文件，可独立横向滚动' : '生成后内容，可独立横向滚动'" class="diff-scroll"
             tabindex="0"
             @scroll="syncVertical(side)">
          <div class="diff-lines">
            <template v-for="entry in visibleRows" :key="entry.key">
              <button v-if="entry.hidden" class="diff-fold" @click="expanded.add(entry.key)">展开 {{ entry.hidden }}
                行未修改代码
              </button>
              <div v-else-if="entry.row" :class="cellClass(entry.row, side)" class="diff-cell">
                <span class="line-number">{{ side === 'left' ? entry.row.leftNo : entry.row.rightNo }}</span>
                <span aria-hidden="true" class="line-sign">{{
                    entry.row.kind === 'change' && entry.row[side] !== undefined ? (side === 'left' ? '−' : '+') : ''
                  }}</span>
                <code class="line-code"><span
                    v-for="(token, index) in entry.row[side + 'Tokens' as 'leftTokens' | 'rightTokens']" :key="index"
                    :class="[token.className, { 'inline-change': token.changed }]">{{ token.text }}</span><span
                    v-if="entry.row[side] !== undefined && !entry.row[side]!.endsWith('\n')"
...
interface RenderRow extends DiffRow {
  leftTokens: Token[];
  rightTokens: Token[]
}
  ...
  try {
    ...
    const walk = (node: Node, classes: string) => {
      if (node.nodeType === Node.TEXT_NODE) {
        append(node.textContent ?? '', classes);
        return
      }
...
const visibleRows = computed<VisibleRow[]>(() => {
  ...
  for (let i = 0; i < all.length;) {
    if (showAll.value || all[i]!.kind === 'change') {
      result.push({key: i, row: all[i]});
      i++;
      continue
    }
    let end = i
    ...
    if (foldEnd - foldStart > 2 && !expanded.value.has(foldStart)) {
      ...
      for (; i < end; i++) result.push({key: i, row: all[i]})
    }
  }
  return result
})
...
.diff-cell {
  ...
  align-items: stretch;
  min-height: 22px;
  font: 12px/22px Consolas, 'Courier New', monospace;
}
...
.diff-removed .line-sign {
  color: #ff7b72;
}

.diff-added .inline-change {
  background: #23863688;
  border-radius: 2px;
}

.diff-removed .inline-change {
  background: #da363388;
  border-radius: 2px;
}

.diff-empty {
  background: repeating-linear-gradient(135deg, #161b22, #161b22 4px, #0d1117 4px, #0d1117 8px);
}
