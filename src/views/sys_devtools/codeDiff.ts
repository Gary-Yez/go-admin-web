...
export function buildRows(before: string, after: string): DiffRow[] {
    ...
    for (let i = 0; i < edits.length;) {
        const edit = edits[i]!
        if (edit.kind === 'same') {
            rows.push({kind: 'same', left: edit.text, right: edit.text, leftNo: leftNo++, rightNo: rightNo++});
            i++
            continue
        }
        ...
        for (let j = 0; j < Math.max(removed.length, added.length); j++) {
            rows.push({
                kind: 'change', left: removed[j], right: added[j],
...
export function changedRanges(before: string, after: string): { left: [number, number][]; right: [number, number][] } {
    ...
    for (const edit of sequenceDiff(Array.from(before.replace(/\n$/, '')), Array.from(after.replace(/\n$/, '')))) {
        if (edit.kind === 'same') {
            left += edit.text.length;
            right += edit.text.length
        } else if (edit.kind === 'remove') {
            append(ranges.left, left, left + edit.text.length);
            left += edit.text.length
        } else {
            append(ranges.right, right, right + edit.text.length);
            right += edit.text.length
        }
    }
    return ranges
