  ...
  setup(props, {attrs, slots, expose}) {
    ...
    function save(reset = false) {
      try {
        ...
        ElMessage.warning('浏览器无法保存列设置，本次调整仍然有效')
      }
      void nextTick(() => tableRef.value?.doLayout())
    }
    ...
    return () => {
      ...
      const columns = nodes.filter(node => node.type === ElTableColumn || (node.type as {
        name?: string
      })?.name === 'ElTableColumn').map(node => {
        ...
        const label = String(data.label || ({
          ...
          index: '序号',
          expand: '展开列'
        } as Record<string, string>)[data.type] || base)
      ...
      return h('div', {class: 'column-table'}, [
        toolbar || props.columnSettings ? h('div', {class: 'column-table-tools'}, [
          ...
          props.columnSettings ? h(ElPopover, {
            trigger: 'click',
            width: 240,
            placement: 'bottom-end',
            visible: settingsOpen.value,
            'onUpdate:visible': (value: boolean) => {
              settingsOpen.value = value
            }
          }, {
            reference: () => h(ElButton, {
              class: ['column-table-trigger', {'is-active': settingsOpen.value}],
              icon: Grid,
              text: true,
              title: '列显示',
              'aria-label': '列显示',
              'aria-expanded': settingsOpen.value,
              disabled: !configurable.length
            }),
            default: () => [
              h('div', {class: 'column-table-heading'}, [
                h('span', '显示列'),
                h(ElButton, {
                  link: true, type: 'primary', size: 'small', onClick: () => {
                    hidden.value = [];
                    save(true)
                  }
                }, () => '恢复默认'),
...
<style scoped>
.column-table :deep(.el-table th.el-table__cell) {
...
.column-table-actions :deep(.el-button + .el-button) {
  margin-left: 0;
}
...
.column-table-trigger :deep(.el-icon) {
  font-size: 17px;
}

.column-table-trigger:not(.is-disabled):hover,
.column-table-trigger.is-active {
  color: var(--el-color-primary);
  background: var(--el-color-primary-light-9);
}

.column-table-trigger:focus-visible {
...
.column-table-options :deep(.el-checkbox) {
  margin-right: 0;
}
</style>
